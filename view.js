    ...
    constructor( container, width, height, x = 0, y = 0, z = -10, scale = 1.0)
    {
        ...
        loader.load(
            ...
            (gltf) => {
                ...
                // 适配模型大小，你可以根据自己的模型调整这个scale参数
                this.object.scale.set(scale, scale, scale);
                this.object.visible = false;
                this.scene.add(this.object);
                console.log('自定义模型加载完成');
            },
            // 加载进度提示
            (xhr) => {
            ...
            // 加载错误处理
            (error) => {
                console.error('模型加载失败，请检查模型文件：', error);
                alert('自定义模型加载失败！请确认你已经把模型文件重命名为my_model.glb，放到assets1文件夹中。');
            }
        );
